...
- The driver applies a scale factor (10000 by default, modified by 
...
factor for X on CS2 to 100000, so the relevant motor record will need an MRES
of 100000
\code
        ...
        for (pDrv = pFirstDrv; pDrv != NULL; pDrv = pDrv->pNext) {
            if (epicsMutexLock( pDrv->controllerMutexId ) == epicsMutexLockOK) {
                for (axis = 0; axis < NAXES; axis++) {
                    ...
                    pAxis->stepSize = 1.0 / (double)defaultSteps;
                }
                epicsMutexUnlock( pDrv->controllerMutexId );
                drvPrint(drvPrintParam, TRACE_FLOW, "  Set scaling factor to %d for CS ref %d.\n", defaultSteps, pDrv->ref);
                status = 0;
            } else {
 ...
 * @param stepsPerUnit Integer number of motor steps per real user unit
 */

int pmacSetCoordStepsPerUnit(int ref, int axis, int stepsPerUnit)
{
    int status = -1;
    PMACDRV_ID pDrv = NULL;
    ...
    if (pFirstDrv != NULL) {
        for (pDrv = pFirstDrv; pDrv != NULL; pDrv = pDrv->pNext) {
            if (ref == pDrv->ref) {
                if (epicsMutexLock( pDrv->controllerMutexId ) == epicsMutexLockOK) {
                    pDrv->axis[axis].stepSize = 1.0 / (double)stepsPerUnit;
                    epicsMutexUnlock( pDrv->controllerMutexId );
                    status = 0;
                    drvPrint(drvPrintParam, TRACE_FLOW, "  Setting axis %d scaling factor to %d.\n", axis, stepsPerUnit);
                } else {
                    drvPrint(drvPrintParam, TRACE_ERROR, "ERROR: Could not lock axis mutex.\n");
                    return status;
                }
